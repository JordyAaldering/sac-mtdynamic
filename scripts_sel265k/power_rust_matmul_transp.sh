...
bench()
{
    ...
    # Warmup
    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/matmul-transp -mt $threads 5 $size > /dev/null

    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/matmul-transp -mt $threads $ITER $size \
