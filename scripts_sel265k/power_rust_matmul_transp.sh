...
bench()
{
    ...
    # Warmup
    numactl -C 0-$(($threads-1)) ./src_rust/target/release/matmul-transp 1 $size $threads > /dev/null

    numactl -C 0-$(($threads-1)) ./src_rust/target/release/matmul-transp $ITER $size $threads \
