...
bench()
{
    ...
    # Warmup
    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/matmul-transp 5 $size $threads > /dev/null

    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/matmul-transp $ITER $size $threads \
