...
bench()
{
    ...
    # Warmup
    numactl -C 0-$(($threads-1)) ./rust/target/release/matmul-naive 1 $size $threads > /dev/null

    numactl -C 0-$(($threads-1)) ./rust/target/release/matmul-naive $ITER $size $threads \
