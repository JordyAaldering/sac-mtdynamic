...
bench()
{
    ...
    # Warmup
    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/matmul-naive 1 $size $threads > /dev/null

    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/matmul-naive $ITER $size $threads \
