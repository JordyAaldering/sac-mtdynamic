...
bench()
{
    ...
    # Warmup
    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/stencil -mt $threads 5 $size > /dev/null

    numactl --interleave all -C 0-$(($threads-1)) ./rust/target/release/stencil -mt $threads $ITER $size \
